...
- 緊急ルートの権限制御は Phase 1 では role を記録する運用とし、403 強制は将来の認証層で実施する
- 緊急変更で許可する対象は chart 閾値・chart set 運用に限定する
- DB スキーマ変更や大量一括変更は緊急例外ルートの対象外とする

緊急変更時の監査と通知:

- 自動必須記録: 変更者（actor）、変更者 role、変更時刻、変更対象、変更差分
...
- 履歴上の緊急変更識別は `ChartsHistory.change_source='emergency_manual'` を正式仕様とする（`is_emergency` 列は Phase 1 では追加しない）。
