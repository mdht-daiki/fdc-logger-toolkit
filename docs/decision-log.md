...
2. リトライは write 系最大 3 回、read 系最大 2 回とし、それ以外の恒久エラーは即時失敗とする
3. バックオフは指数型（100ms, 300ms, 900ms）を基本とし、各回に小さなジッターを付与する
4. 接続レベルの busy timeout は 3000ms を標準値とする
5. 要求全体の上限時間は write 系 10 秒、read 系 5 秒を目安とする
6. 障害時はトランザクション rollback を前提に、失敗分類（lock timeout/validation/unexpected）ごとの再実行手順を runbook 化する
